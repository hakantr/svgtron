...
5. **Kod paneli (`kod-paneli.ts`) → CodeMirror 6 → ✅ ALINDI (commit 6845250).**
   - *Neden al:* contenteditable mini-editör (TK-26) gerçekten kırılgan (Enter/ağaç-
     ...
     vurgu/satır no/büyük-dosya başarımında sağlam.
   - *Nasıl (çift-yönlü senkron korundu):* Eski `data-kimlik` span ağacı yerine yeni
     saf `kod-metin.ts` — `kodMetni(kök)` → **metin + kimlik→karakter-aralığı haritası**;
     `konumdakiKimlik` bir konumu kapsayan EN İÇTEKİ düğümü verir. Seçili düğüm CM6
     **decoration** (`seciliAlan` StateField) ile vurgulanır; koda tıklayınca
     `posAtCoords`→`konumdakiKimlik`→seç (Shift=çoklu). "Uygula" yine `KodUygulaKomutu`
     (İlke 2, geri-alınır); CM6'nın editör-içi undo'su yalnız metin düzenlemesi için.
     Kirli/odakta iken model değişse de metin EZİLMEZ. Eski `kod-goster.ts` silindi.
   - *Bedel:* Bundle ~880 KB→1.78 MB (codemirror@6 + lang-html). Masaüstü (yerel) için
     kabul edilebilir. Birim test: `kod-metin` 3/3 (aralık/en-içteki/kara-liste).
   - *Sınır:* Panel UX'i (vurgu/tıkla-seç/syntax render) GÖRSEL → gözle teyit gerekir
     (§5); araç/etkileşim doğru kuruldu, render kullanıcı doğrulamasına bağlı.
...
**Bağımlılık:** `css-tree@^3` + `@types/css-tree` (CSS editör); `codemirror@^6` +
`@codemirror/{state,view,commands,language,lang-html}` (kod paneli). Hepsi saf JS,
native build/postinstall yok.

**Güncel durum (bu rapor için aksiyon):** Raporun "benimse" dediğim İKİ maddesi de
uygulandı — **css-tree** (CSS editör) ve **CodeMirror 6** (kod paneli). Kalan dört öneri
(path adapter, renk/culori, SVGO, DOMPurify) gerekçeyle ertelendi/reddedildi (yukarıda);
yeni bir ihtiyaç/özellik tetiklemeden alınmayacaklar.
