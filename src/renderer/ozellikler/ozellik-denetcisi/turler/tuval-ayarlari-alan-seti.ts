...
function sayiAlan(etiket: string, mevcut: string, uygula: (n: number) => void): TemplateResult {
  return html`
    <label>${etiket}</label>
    <input
      type="number"
...
function metinAlan(etiket: string, mevcut: string, uygula: (v: string) => void): TemplateResult {
  return html`
    <label>${etiket}</label>
    <input
      type="text"
...
function viewBoxParcala(deger: string | undefined): [number, number, number, number] | null {
  if (!deger) return null;
  const p = deger.trim().split(/[\s,]+/).map(Number);
...
function tuvalBolumu(baglam: AlanSetiBaglami): TemplateResult {
  ...
  return html`
    ...
    <div class="izgara">
      ${metinAlan(t('denetci.geo.genislik'), oz.get('width') ?? '', (v) => yaz('width', v))}
      ${metinAlan(t('denetci.geo.yukseklik'), oz.get('height') ?? '', (v) => yaz('height', v))}
    </div>
