...
function yaz(baglam: AlanSetiBaglami, etiket: string, deger: string): void {
  ...
  if (mevcut) {
    ...
    baglam.komut(new DugumEkleKomutu(baglam.belge, baglam.dugum, dugumOlustur(etiket, {}, [], deger), 0));
  }
}
    ...
    return html`
      <div class="alan">
        ...
        <input
          type="text"
          .value=${baslik}
          @change=${(e: Event) => yaz(baglam, 'title', (e.target as HTMLInputElement).value)}
        />
      </div>
      <div class="alan">
        ...
        <input
          type="text"
          .value=${aciklama}
          @change=${(e: Event) => yaz(baglam, 'desc', (e.target as HTMLInputElement).value)}
        />
      </div>
      <div class="alan">
        ...
        <input
          ...
          @change=${(e: Event) => baglam.yaz('aria-label', (e.target as HTMLInputElement).value)}
        />
      </div>
    `;
  },
