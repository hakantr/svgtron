...
const gorunumAlanSeti: AlanSeti = {
  ...
  render: (baglam) => {
    ...
    const strokeW = (
      h?.strokeWidth ||
      dugum.oznitelikler.get("stroke-width") ||
      "1"
    ).replace("px", "");
      ...
      <div class="alan">
        ...
        <div class="satir">
          <input
            ...
            @change=${(e: Event) =>
              stilYaz("opacity", (e.target as HTMLInputElement).value)}
          />
          <span class="deger"
            >${(Number.isFinite(opNum) ? opNum : 1).toFixed(2)}</span
          >
        </div>
      </div>
    `;
...
function renkAlani(
  ...
  const boyaYaz = (deger: string): void => {
    const stilKomut = stilUygulaKomutu(
      baglam.belge,
      baglam.dugum,
      ozellik,
      deger,
    ...
    baglam.komut(
      temizle
        ? new BilesikKomut("boya değiştir", [stilKomut, temizle])
        : stilKomut,
    );
  };
  ...
  const uygula = (b: BoyaDegeri): void => {
    if (b.tip === "gradyan")
      baglam.komut(
        gradyanKomutu(baglam.belge, baglam.dugum, hamDeger, b, ozellik),
      );
