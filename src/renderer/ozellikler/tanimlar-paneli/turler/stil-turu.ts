...
import { dugumOlustur, gez, tumSiniflar, type Dugum } from '../../../../cekirdek/belge/model/dugum';
import type { Belge } from '../../../../cekirdek/belge/belge';
import type { Komut } from '../../../../cekirdek/komutlar/komut';
import { BilesikKomut, DugumEkleKomutu } from '../../../../cekirdek/komutlar/dugum-komutlari';
...
/**
 * Stil sınıfları kaynak türü (CLAUDE.md Faz E, §8.1, §10.5).
 *
...
kaynakTuruKayitDefteri.kaydet({
  ...
  listele(belge) {
    const set = new Set<string>();
    for (const s of stilDugumleri(belge)) for (const c of siniflariCikar(s.metin ?? '')) set.add(c);
  ...
  uygula(belge, dugumler, kaynakId): Komut | null {
    ...
    const komutlar = dugumler.map((d) => {
      const mevcut = (d.oznitelikler.get('class') ?? '').split(/\s+/).filter(Boolean);
  ...
  olustur(belge): Komut {
    ...
    for (const s of stilDugumleri(belge)) for (const c of siniflariCikar(s.metin ?? '')) mevcut.add(c);
    let n = 1;
    ...
    if (stil) {
      komutlar.push(new MetinKomutu(belge, stil, `${(stil.metin ?? '').trimEnd()}\n${kural}\n`));
    } else {
      ...
      komutlar.push(new DugumEkleKomutu(belge, defs, dugumOlustur('style', {}, [], `\n${kural}\n`)));
    }
    return new BilesikKomut('sınıf oluştur', komutlar);
  },

  sil(belge, kaynakId): Komut | null {
    const stil = stilDugumleri(belge).find((s) => siniflariCikar(s.metin ?? '').includes(kaynakId));
    if (!stil) return null;
    const kacis = kaynakId.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const yeni = (stil.metin ?? '').replace(new RegExp(`\\.${kacis}\\s*\\{[^}]*\\}\\s*`, 'g'), '');
...
function sinifStilDugumu(belge: Belge, kaynakId: string): Dugum | null {
  return stilDugumleri(belge).find((s) => siniflariCikar(s.metin ?? '').includes(kaynakId)) ?? null;
}
...
kaynakGorunumKaydet({
  ...
  onizleme: (belge, id) => {
    ...
    return onizlemeSvg(`${dugumSerile(stil)}<rect width="30" height="18" class="${id}"/>`);
  },
  duzenle: ({ belge, kaynakId, komut }) => {
    const stil = sinifStilDugumu(belge, kaynakId);
    if (!stil) return nothing;
    const selector = `.${kaynakId}`;
    const oku = (ozellik: string): string | null => cssKuralOku(stil.metin ?? '', selector, ozellik);
    const yaz = (ozellik: string, deger: string): void =>
      komut(new MetinKomutu(belge, stil, cssKuralYaz(stil.metin ?? '', selector, ozellik, deger)));
    return html`
      <div class="satir">
        <label>fill</label>
        <input
          ...
          @change=${(e: Event) => yaz('fill', (e.target as HTMLInputElement).value)}
        />
      </div>
      <div class="satir">
        <label>stroke</label>
        <input
          ...
          @change=${(e: Event) => yaz('stroke', (e.target as HTMLInputElement).value)}
        />
      </div>
      <div class="satir">
        <label>opacity</label>
        <input
          ...
          max="1"
          .value=${oku('opacity') ?? '1'}
          @change=${(e: Event) => yaz('opacity', (e.target as HTMLInputElement).value)}
        />
      </div>
    `;
  },
