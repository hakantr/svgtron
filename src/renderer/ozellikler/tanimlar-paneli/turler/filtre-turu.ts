...
/**
 * Filter kaynak türü (CLAUDE.md Faz C, §8.1) — "kaynak türü deseni"nin ilk uçtan
...
kaynakTuruKayitDefteri.kaydet({
  ...
  uygula(belge, dugumler, kaynakId): Komut | null {
    ...
    return new BilesikKomut(
      'filtre uygula',
      dugumler.map((d) => stilUygulaKomutu(belge, d, 'filter', `url(#${kaynakId})`)),
    );
  },
    ...
    return html`
      ...
      ${renkVar
        ? html`<div class="satir">
            ...
            <input
              type="color"
              .value=${hexNorm(primitif.oznitelikler.get('flood-color') ?? '#000000')}
              @change=${(e: Event) => yaz('flood-color', (e.target as HTMLInputElement).value)}
            />
          </div>`
        : nothing}
    `;
