...
/**
 * mask (maske) kaynak türü (AGENTS.md Faz G+, §8.1, §10.6). Uygulama stratejisi
...
function maskeDugumu(belge: Belge, id: string): Dugum | null {
  for (const d of gez(belge.kok))
    if (d.etiket === "mask" && d.oznitelikler.get("id") === id) return d;
  return null;
}
...
kaynakTuruKayitDefteri.kaydet({
  ...
  uygula(belge, dugumler, kaynakId): Komut | null {
    ...
    return new BilesikKomut(
      "maske uygula",
      dugumler.map((d) =>
        stilUygulaKomutu(belge, d, "mask", `url(#${kaynakId})`),
      ),
    );
  },
...
kaynakGorunumKaydet({
  ...
  onizleme: (belge, id) =>
    defsOnizleme(
      belge,
      id,
      `<rect width="30" height="18" fill="#7aa7e6" mask="url(#${id})"/>`,
    ),
});
