...
/**
 * Pattern (desen) kaynak türü (CLAUDE.md Faz G+, §8.1, §10.5). "Yeni tür = yeni
...
function desenDugumu(belge: Belge, id: string): Dugum | null {
  for (const d of gez(belge.kok)) if (d.etiket === 'pattern' && d.oznitelikler.get('id') === id) return d;
  return null;
}
    ...
    return html`
      <div class="satir">
        ...
        <input
          ...
          @change=${(e: Event) => yaz('width', (e.target as HTMLInputElement).value)}
        />
      </div>
      <div class="satir">
        <label>height</label>
        <input
          ...
          @change=${(e: Event) => yaz('height', (e.target as HTMLInputElement).value)}
        />
      </div>
    `;
  },
