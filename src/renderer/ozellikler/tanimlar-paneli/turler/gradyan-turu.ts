...
function gradyanAci(grad: Dugum): number {
  const s = (ad: string, v: number) => parseFloat(grad.oznitelikler.get(ad) ?? String(v)) || 0;
...
function gradyanAciYaz(belge: Belge, grad: Dugum, derece: number): Komut {
  ...
  if (grad.oznitelikler.get('gradientUnits') === 'userSpaceOnUse') {
    const oku = (ad: string, v: number) => parseFloat(grad.oznitelikler.get(ad) ?? String(v)) || 0;
    const ox1 = oku('x1', 0);
    const oy1 = oku('y1', 0);
    const ox2 = oku('x2', 0);
    const oy2 = oku('y2', 0);
...
/**
 * Gradyan kaynak türü (CLAUDE.md Faz D, §8.1, §10.5) — `linearGradient` /
...
kaynakTuruKayitDefteri.kaydet({
  ...
  listele(belge) {
    ...
    for (const d of gez(belge.kok)) {
      ...
      if (GRADYAN.has(d.etiket) && id) {
        ogeler.push({ id, etiket: `${id} (${d.etiket === 'radialGradient' ? 'radyal' : 'doğrusal'})` });
      }
    }
    return ogeler;
  },
  ...
  uygula(belge, dugumler, kaynakId): Komut | null {
    ...
    return new BilesikKomut(
      'gradyan uygula',
      dugumler.map((d) => stilUygulaKomutu(belge, d, 'fill', `url(#${kaynakId})`)),
    );
  },
...
kaynakGorunumKaydet({
  turId: 'gradyan',
  onizleme: (belge, id) => defsOnizleme(belge, id, `<rect width="30" height="18" fill="url(#${id})"/>`),
  duzenle: ({ belge, kaynakId, komut }) => {
    ...
    const durakEkle = (): void =>
      komut(new DugumEkleKomutu(belge, grad, dugumOlustur('stop', { offset: '1', 'stop-color': '#000000' })));
    return html`
      ${grad.etiket === 'linearGradient'
        ? html`<div class="satir">
            ...
            <input
              ...
              @change=${(e: Event) =>
                komut(gradyanAciYaz(belge, grad, Number((e.target as HTMLInputElement).value) || 0))}
            />
          </div>`
        : ''}
      ${duraklar.map(
        (durak) => html`<div class="satir">
          <input
            type="color"
            .value=${hexNorm(durak.oznitelikler.get('stop-color') ?? '#000000')}
            @change=${(e: Event) => yaz(durak, 'stop-color', (e.target as HTMLInputElement).value)}
          />
          <input
            type="number"
            step="0.05"
            ...
            @change=${(e: Event) => yaz(durak, 'offset', (e.target as HTMLInputElement).value)}
          />
          ${duraklar.length > 2
            ? html`<button
                class="kucuk-sil"
                title=${t('tanimlar.sil')}
                @click=${() => komut(new DugumCikarKomutu(belge, grad, durak))}
              >
                ✕
              </button>`
            : ''}
      ...
      <button class="ekle" @click=${durakEkle}>${t('tanimlar.durakEkle')}</button>
    `;
  },
});
