...
import { yoluTersCevir, yoluBasitlestir } from '../../../cekirdek/belge/model/yol-duzenleme';
 ...
 */
function pathlariDonustur(baglam: MenuBaglami, donustur: (s: Segment[]) => Segment[], etiket: string): void {
  ...
  for (const d of baglam.secim.secililer) {
    ...
    komutlar.push(new OznitelikDegistirKomutu(belge, d, 'd', yoluYaz(donustur(segs))));
  }
  if (komutlar.length) baglam.gecmis.calistir(new BilesikKomut(etiket, komutlar));
}
...
menuKayitDefteri.kaydet({
  ...
  sira: 5,
  calistir: (b) => pathlariDonustur(b, (s) => yoluBasitlestir(s, 1.5), 'yolu basitleştir'),
});
