import { gez, dugumOlustur, type Dugum } from '../../../cekirdek/belge/model/dugum';
...
/**
 * Optimize / temizle (CLAUDE.md §11.4) — "temiz" dışa-aktarım profilinin (İlke 8)
...
function kullanilmayanTanimlar(belge: Belge): { dugum: Dugum; ebeveyn: Dugum }[] {
...
const YUVARLANACAK = new Set([
  ...
  'x1', 'y1', 'x2', 'y2', 'dx', 'dy', 'offset', 'stroke-width', 'font-size',
]);
...
function yuvarlamaKomutlari(belge: Belge, basamak: number): Komut[] {
  ...
  const yaz = (d: Dugum, ad: string, yeni: string): void => {
    if (yeni !== d.oznitelikler.get(ad)) komutlar.push(new OznitelikDegistirKomutu(belge, d, ad, yeni));
  };
  for (const d of gez(belge.kok)) {
...
function temizle(baglam: MenuBaglami): void {
  ...
  if (komutlar.length) baglam.gecmis.calistir(new BilesikKomut('kullanılmayanları temizle', komutlar));
}
...
function yuvarla(baglam: MenuBaglami): void {
  ...
  if (komutlar.length) baglam.gecmis.calistir(new BilesikKomut('koordinatları yuvarla', komutlar));
}
...
function dejenereKomutlari(belge: Belge): { komutlar: Komut[]; cizgi: number; sil: number } {
  const komutlar: Komut[] = [];
  let cizgi = 0;
  let sil = 0;

  const yuru = (dugum: Dugum, ebeveyn: Dugum | null, tanimda: boolean): void => {
