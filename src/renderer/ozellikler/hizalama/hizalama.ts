...
function referansKutusu(
  ...
  if (ref === "belge") {
    ...
    const hedef = artboard
      ? cizimErisimi.eleman(artboard.kimlik)
      : cizimErisimi.eleman(belge.kok.kimlik);
    return hedef instanceof SVGGraphicsElement
      ? kenarlar(hedef.getBoundingClientRect())
      : null;
  }
...
export function hizala(
  ...
  if (mod === "dagit-yatay" || mod === "dagit-dikey") {
    ...
    const merkez = (r: DOMRect) =>
      yatay ? (r.left + r.right) / 2 : (r.top + r.bottom) / 2;
    ...
    const r =
      referansKutusu(belge, secim, kayit, hizalaReferans.mod) ??
      topluKutu(kayit.map((k) => k.r));
