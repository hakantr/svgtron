import { svg } from 'lit';
import { aracKayitDefteri, type Arac, type AracBaglami, type TuvalNoktasi } from '../arac';
...
function dDizesi(nokta: readonly TuvalNoktasi[], ipucu?: TuvalNoktasi, kapali = false): string {
  if (nokta.length === 0) return '';
  const parcalar = [`M ${say(nokta[0]!.x)} ${say(nokta[0]!.y)}`];
  for (let i = 1; i < nokta.length; i++) parcalar.push(`L ${say(nokta[i]!.x)} ${say(nokta[i]!.y)}`);
...
function bitir(baglam: AracBaglami, kapali: boolean): void {
  ...
  if (belge && noktalar.length >= 2) {
    const dugum = dugumOlustur('path', { ...VARSAYILAN, d: dDizesi(noktalar, undefined, kapali) });
...
const kalemAraci: Arac = {
  ...
  bas(olay, baglam) {
    ...
    if (noktalar.length >= 2 && ilkEkran && Math.hypot(olay.clientX - ilkEkran.x, olay.clientY - ilkEkran.y) <= KAPAT_ESIGI) {
