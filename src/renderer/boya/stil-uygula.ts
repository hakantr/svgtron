import { dugumOlustur, gez, benzersizSinif, type Dugum } from '../../cekirdek/belge/model/dugum';
...
function nesneStilSinifi(dugum: Dugum): string | null {
  return (dugum.oznitelikler.get('class') ?? '').split(/\s+/).find((c) => c.startsWith(ONEK)) ?? null;
}
...
function belgeSinifTanimliMi(belge: Belge, cls: string): boolean {
  ...
  return cls.split(/\s+/).some((c) => c && new RegExp(`\\.${kacis(c)}(?![\\w-])`).test(metin));
}
...
function inlineKomutu(belge: Belge, dugum: Dugum, ozellik: string, deger: string): Komut {
...
function cssKomutu(belge: Belge, dugum: Dugum, ozellik: string, deger: string): Komut {
  ...
  if (!sinif) {
    ...
    mevcut.push(sinif);
    komutlar.push(new OznitelikDegistirKomutu(belge, dugum, 'class', mevcut.join(' ')));
  }
  ...
  const st = styleDugumu(belge);
  if (st) {
    komutlar.push(new MetinKomutu(belge, st, cssKuralYaz(st.metin ?? '', selector, ozellik, deger)));
  } else {
    ...
    komutlar.push(new DugumEkleKomutu(belge, defs, dugumOlustur('style', {}, [], metin)));
  }
  ...
  if (inlineStyle && stilOku(inlineStyle, ozellik) != null) {
    komutlar.push(new OznitelikDegistirKomutu(belge, dugum, 'style', stilAyarla(inlineStyle, ozellik, '')));
  }

  return komutlar.length === 1 ? komutlar[0]! : new BilesikKomut('stil (css)', komutlar);
}
 ...
 */
export function stilUygulaKomutu(belge: Belge, dugum: Dugum, ozellik: string, deger: string): Komut {
  const istenen = stilYazimModu.mod;
  const mod = istenen === 'otomatik' ? otomatikSec(belge, dugum) : istenen;
  return mod === 'css' ? cssKomutu(belge, dugum, ozellik, deger) : inlineKomutu(belge, dugum, ozellik, deger);
}
