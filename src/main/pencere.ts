...
/**
 * Ana pencereyi oluşturur ve AGENTS.md §3 güvenlik modelini ZORUNLU kılar.
 *
...
export function pencereOlustur(): BrowserWindow {
  ...
  const durumGonder = (): void => {
    pencere.webContents.send(
      KANALLAR.pencereDurumDegisti,
      pencere.isMaximized(),
    );
  };
  ...
  pencere.webContents.on("will-frame-navigate", (olay) =>
    icGezinme(olay, olay.url),
  );
