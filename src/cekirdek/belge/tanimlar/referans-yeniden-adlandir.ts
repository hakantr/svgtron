...
/**
 * Güvenli yeniden adlandırma (AGENTS.md §8.2, İlke 7).
 *
...
function idReferansiniYenidenAdlandir(
  belge: Belge,
  dugum: Dugum,
  eski: string,
  yeni: string,
      ...
      if (urlTest.test(deger)) {
        komutlar.push(
          new OznitelikDegistirKomutu(
            ...
            dugum,
            "style",
            deger.replace(urlGlobal, `$1${yeni}$2`),
          ),
        );
      }
...
function sinifReferansiniYenidenAdlandir(
  belge: Belge,
  dugum: Dugum,
  eski: string,
  yeni: string,
  ...
  return [
    new OznitelikDegistirKomutu(belge, dugum, "class", yeniListe.join(" ")),
  ];
}
...
function tanimIdKomutu(belge: Belge, eski: string, yeni: string): Komut | null {
  for (const d of gez(belge.kok)) {
    if (d.oznitelikler.get("id") === eski)
      return new OznitelikDegistirKomutu(belge, d, "id", yeni);
  }
  return null;
}

/** class-kaynağın `<style>` kurallarındaki seçiciyi (`.eski` → `.yeni`) değiştiren komutlar. */
function sinifTanimKomutlari(
  belge: Belge,
  eski: string,
  yeni: string,
): Komut[] {
    ...
    if (sel.test(metin)) {
      komutlar.push(
        new MetinKomutu(
          belge,
          d,
          metin.replace(new RegExp(`\\.${esc}(?![\\w-])`, "g"), `.${yeni}`),
        ),
      );
    }
  }
...
function idKullanimda(belge: Belge, id: string): boolean {
  for (const d of gez(belge.kok))
    if (d.oznitelikler.get("id") === id) return true;
  return false;
}
...
export function kaynakYenidenAdlandir(
  ...
  const kullananlar = sinifMi
    ? idx.sinifiKullananlar(eskiId)
    : idx.kullananlar(eskiId);

  const tanimKomutlari = sinifMi
    ? sinifTanimKomutlari(belge, eskiId, yeni)
    : [tanimIdKomutu(belge, eskiId, yeni)].filter(
        (k): k is Komut => k !== null,
      );
  ...
  return {
    komut:
      hepsi.length === 1
        ? hepsi[0]!
        : new BilesikKomut("yeniden adlandır", hepsi),
