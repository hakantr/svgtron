...
/**
 * Kaynak referans temizliği (güvenli silme — AGENTS.md §8.2, İlke 7).
 *
...
function idReferansiniTemizle(belge: Belge, dugum: Dugum, id: string): Komut[] {
  ...
  for (const [ad, deger] of dugum.oznitelikler) {
    if (ad === "style") {
      ...
      (ad === "href" || ad === "xlink:href") &&
      (hashEsit.test(deger) || urlTest.test(deger))
    ) {
...
function sinifReferansiniTemizle(
  belge: Belge,
  dugum: Dugum,
  sinif: string,
): Komut[] {
...
export function kaynakReferansTemizle(
  ...
  const dugumler = sinifMi
    ? idx.sinifiKullananlar(kaynakId)
    : idx.kullananlar(kaynakId);
  const komutlar: Komut[] = [];
