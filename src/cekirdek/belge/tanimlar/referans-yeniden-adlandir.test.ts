...
test('sınıf yeniden adlandırma: <style> kuralı + class atıfları', () => {
  const style = dugumOlustur('style', {}, [], '.cls { fill: red; }\n.other { stroke: blue; }');
  const rect = dugumOlustur('rect', { class: 'cls other' });
  const kok = dugumOlustur('svg', {}, [dugumOlustur('defs', {}, [style]), rect]);
  const belge = sahteBelge(kok);
...
test('çakışma ve geçersiz ad reddedilir (komut null)', () => {
  ...
  assert.equal(kaynakYenidenAdlandir(belge, 'g1', '1bad', false).hata, 'gecersiz');
  assert.equal(kaynakYenidenAdlandir(belge, 'g1', 'g1 g3', false).hata, 'gecersiz'); // boşluk
...
test('benzer önekli id yanlış eşleşmez (#g1 vs #g10)', () => {
  ...
  const kok = dugumOlustur('svg', {}, [dugumOlustur('defs', {}, [g1, g10]), r1, r10]);
  const belge = sahteBelge(kok);
