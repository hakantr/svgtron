...
/**
 * Yol (path) düzenleme işlemleri (CLAUDE.md §11.2) — saf, Electron'dan habersiz.
...
function altYoluTersCevir(alt: AltYol): Segment[] {
  const ss = alt.segs;
  const a: Nokta[] = ss.map((s) => anchor(s)).filter((p): p is Nokta => p !== null);
  const n = a.length - 1;
  ...
  for (let k = n; k >= 1; k--) {
    ...
    switch (seg.tip) {
      ...
      case 'M':
        ...
        break;
      case 'C':
        sonuc.push({ tip: 'C', c1: { ...seg.c2 }, c2: { ...seg.c1 }, p: hedef });
        break;
      case 'Q':
 ...
 */
export function yoluBasitlestir(segs: readonly Segment[], tolerans = 1): Segment[] {
  const sonuc: Segment[] = [];
  for (const alt of altYollaraBol(segs)) {
    const pts = alt.segs.map((s) => anchor(s)).filter((p): p is Nokta => p !== null);
    const sade = rdp(pts, tolerans);
    if (sade.length === 0) continue;
    sonuc.push({ tip: 'M', p: { ...sade[0]! } });
    for (let i = 1; i < sade.length; i++) sonuc.push({ tip: 'L', p: { ...sade[i]! } });
    if (alt.kapali) sonuc.push({ tip: 'Z' });
  }
  return sonuc;
}
