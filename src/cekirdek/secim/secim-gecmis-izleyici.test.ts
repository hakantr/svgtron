...
function sahteKomut(
  etiket: string,
): Komut & { uygulandiSayisi: number; geriAlindiSayisi: number } {
  return {
    etiket,
    uygulandiSayisi: 0,
    geriAlindiSayisi: 0,
...
test("§9.6 e — seçim adımları 5 ile sınırlı, düzenleme adımları korunur", () => {
  ...
  const secimSayisi = giris.filter(
    (g) => !g.etiket.startsWith("düzenleme"),
  ).length;
  assert.equal(secimSayisi, 5, "en çok 5 seçim adımı tutulmalı");
  assert.equal(
    giris.filter((g) => g.etiket.startsWith("düzenleme")).length,
    2,
    "düzenleme adımları korunmalı",
  );
...
test("yeni adım gelince ileri (redo) dalı atılır", () => {
  ...
  secim.ekle(n("a"));
  assert.equal(
    gecmis.ileriAlinabilir,
    false,
    "yeni adım sonrası redo dalı kalmamalı",
  );
});
...
test("Bug B — undo sonrası yeni TEK seçim, bayat redo dalını atar (§9.6 g)", () => {
  ...
  assert.equal(
    gecmis.ileriAlinabilir,
    false,
    "yeni tek seçim redo dalını atmalı",
  );
});
...
test("Bug C — flush edilmiş tek-eleman adımına dönüp uzatmak YİNELENEN adım üretmez", () => {
  ...
  assert.deepEqual(
    etiketler(gecmis),
    ["daire seçildi", "x seçildi"],
    'yinelenen "daire seçildi" olmamalı',
  );
});
